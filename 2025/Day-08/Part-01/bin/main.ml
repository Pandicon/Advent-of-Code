...
let connections_to_make = if run_example then 10 else 1000
...
let solve (input : string) : int =
  ...
  let rec build_circuits
      ...
      (repetitions_left : int) (acc : position list list) : position list list =
    if repetitions_left = 0 then acc
    else
      ...
      | (pos1, pos2, _dist) :: t ->
          build_circuits t (repetitions_left - 1)
            (insert_positions pos1 pos2 acc [])
  in
  let connections_lengths_sorted =
    build_circuits distances connections_to_make []
    |> List.map (fun (connections : position list) ->
           connections |> List.length)
    |> List.sort (fun (len1 : int) (len2 : int) -> len2 - len1)
  in
  match connections_lengths_sorted with
  | f :: s :: t :: _ -> f * s * t
  | [ f; s ] -> f * s
  | [ f ] -> f
  | [] -> 1
