fn main() {
	...
	let input = std::fs::read_to_string(input_path).expect("Should have been able to read the file");

	let max_calories = input
		.split("\n\n")
		.map(|elf_inv| elf_inv.split("\n").map(|snack_calories| snack_calories.trim().parse::<i32>().unwrap()).sum::<i32>())
		.max()
		.unwrap();
