	...
	for type_locs in locations.values() {
		for i in 0..type_locs.len() {
			for j in (i+1)..type_locs.len() {
				...
				let x_a_1 = x_2 + (x_2 - x_1);
				let y_a_1 = y_2 + (y_2 - y_1);
				let x_a_2 = x_1 - (x_2 - x_1);
				let y_a_2 = y_1 - (y_2 - y_1);
				if y_a_1 >= 0 && y_a_1 < chars_map.len() as isize {
					let y_a_1 = y_a_1 as usize;
					if x_a_1 >= 0 && x_a_1 < chars_map[y_a_1].len() as isize {
						let x_a_1 = x_a_1 as usize;
						occupied[y_a_1][x_a_1] = true;
					}
				}
				if y_a_2 >= 0 && y_a_2 < chars_map.len() as isize {
					let y_a_2 = y_a_2 as usize;
					if x_a_2 >= 0 && x_a_2 < chars_map[y_a_2].len() as isize {
						let x_a_2 = x_a_2 as usize;
						occupied[y_a_2][x_a_2] = true;
					}
				}
			}
		}
