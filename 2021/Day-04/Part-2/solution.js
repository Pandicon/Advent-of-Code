...
for(const board of boardsRaw) {
	...
	boardWinStates.push(false);
};

const winnerInfo = getLastWinningBoard(nums, boards, boardsBools, boardHeight, boardWidth, boardWinStates);
...
function getLastWinningBoard(nums, boards, boardsBools, boardHeight, boardWidth, boardWinStates) {
	for(const num of nums) {
		for(const boardIndex in boards) {
			...
			if(isBoardWinning(numIndex, boardsBools[boardIndex], boardHeight, boardWidth)) {
				boardWinStates[boardIndex] = true;
				if(boardWinStates.every(x => x)) {
					return [board, boardsBools[boardIndex], num];
				};
			};
		};
	};
